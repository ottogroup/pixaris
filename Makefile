...
deploy-gradio-app-to-app-engine:
	...
	@project_id=$$(poetry run python -c "import yaml; print(yaml.safe_load(open('config.yaml'))['gcp_project_id_for_app_engine'])")
	@service_account=$$(poetry run python -c "import yaml; print(yaml.safe_load(open('config.yaml'))['gcp_service_account_for_appengine_deployment'])") && \
